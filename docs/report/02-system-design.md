...
2026-07-22 기준 단일 Django ASGI 구조에서 사용자·상품·검색·채팅·알림·거래·모의 이체·관리·후기·회원 탈퇴 기능과 PostgreSQL 마이그레이션을 구현했습니다. PostgreSQL은 영속 권위이고 Redis는 재구축 가능한 전달·presence projection입니다. 자동 테스트, 마이그레이션, 백업→빈 DB 복원, 브라우저·axe 검증 결과는 종합 검증 근거로 분리합니다.
