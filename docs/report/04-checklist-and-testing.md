...
- `PASS`: 실제 실행 결과와 근거가 남아 있는 상태입니다.
- `FAIL`: 실제 실행 결과가 기대와 다른 상태입니다.
- `미검증`: 테스트 방법은 정했지만 아직 실행하지 않은 상태입니다.
- `구현 예정`: 테스트할 기능이 아직 없는 상태입니다.
- `재검증 필요`: 코드나 환경이 바뀌어 같은 조건에서 이전 결과를 다시 확인해야 하는 상태입니다.
...
금액·cap fixture에는 직접 balance UPDATE를 사용하지 않습니다. test maintainer는 실제 posting 서비스로 합계 0 `COMPENSATION` journal과 온라인 잔액을 한 트랜잭션에서 만들어 경계 잔액을 준비합니다. 시작 전 reconciliation 불일치가 0건인지 확인하고, 성공·거부·경합 뒤에도 같은 대사를 수행합니다.
...
정렬·페이지 테스트는 예상 상품 ID 순서를 고정해 비교합니다. 비노출 만료 경계는 애플리케이션 시각이 아니라 PostgreSQL 현재 시각으로 검증합니다. 목록 건수와 실제 행에 같은 가시성 조건이 적용되는지도 확인합니다.
...
- 2차 모델의 전진 적용과 직전 상태 롤백은 실제 PostgreSQL에서 검증하며, 실패 중간 상태나 데이터 유실이 없어야 합니다.
- 검색·관리·이체의 모든 HTTP 상태 변경에서 CSRF, 서버 측 권한, 일반화된 오류, 민감정보 비기록을 다시 확인합니다.
...
2026-07-16에 현재 1차 애플리케이션을 대상으로 아래 검증을 실행했습니다.
...
검증 대상은 통합 leader head `38a5c87`에서 시작한 작업 트리입니다. 전체 matrix는 한 번 실행했으며, 실패는 숨기지 않고 원인을 수정한 뒤 실패 범위만 다시 실행했습니다.
