...
|---|---|---|---|---|---|---|
| `FR-USER-01` | 회원가입·로그인 | 정상·중복·잘못된 아이디와 비밀번호로 요청 | 유효한 사용자만 생성·로그인 | 사용자 모델과 아이디 정책 골격만 있음 | 구현 예정 | URL·화면·인증 흐름 테스트 필요 |
| `FR-USER-03` | 본인 소개글·비밀번호 변경 | 본인과 다른 사용자 세션으로 변경 시도 | 본인만 허용 | 변경 필드 allowlist 단위 테스트 PASS, 종단 서비스 없음 | 미검증 | `tests/unit/accounts_catalog/` |
...
| `SR-SESSION-01` | CSRF | 토큰 없는 상태 변경 요청과 위조 Origin 요청 | 403으로 차단 | 운영 HTTPS 출처 설정 검증 PASS, 실제 요청 미실행 | 미검증 | Django client·브라우저 테스트 필요 |
...
이 결과는 골격과 정책 계약의 근거입니다. 아직 구현하지 않은 회원가입 화면, 상품 CRUD, WebSocket consumer, 신고 생성 서비스, 검색, 관리자, 모의 이체는 PASS로 해석하지 않습니다. 상세 명령과 남은 검증은 [테스트 근거](appendix/test-evidence.md)에 기록했습니다.
