...
2차 요구사항·설계 문서는 작성했지만 검색·관리·모의 이체 제품 코드는 아직 없습니다. 따라서 아래 항목은 모두 `구현 예정`이며 실제 실행 전에는 `PASS`로 올리지 않습니다. 구현 단계에서는 [요구사항 분석](01-requirements.md)에 정의된 경계값과 [시스템 설계](02-system-design.md)의 권한·트랜잭션 계약을 기대값으로 사용합니다.
...
| 계획 Test-ID | 연결 정책 | 유형 | 실행할 조건 | 기대 결과 | 상태 |
|---|---|---|---|---|---|
| `C2-SRCH-VIS-001` | `검색-공개-01` | 권한·음성 | 같은 검색어를 공개/비노출 상품과 소유자 소개·신고·채팅·관리 메모에 각각 두고 비회원·회원·소유자·관리자로 검색 | 일반 검색은 공개 상품 제목·설명만 일치하고 비노출·금지 도메인은 결과·건수에 없으며 응답 필드는 상품 ID·제목·가격·상태·이미지·시각 allowlist뿐 | 구현 예정 |
| `C2-SRCH-INPUT-001` | `검색-입력-01` | 경계·입력 | migration backfill, 생성/수정 서비스, raw DB에 조합/분해 한글을 각각 저장하고 NFC/NFD 검색어·100/101자·C0/C1을 조합 | 저장값은 NFC, raw NFD는 DB CHECK 거부, 저장/검색어 네 조합은 같은 결과, 정규화 뒤 101자·제어문자는 상품 쿼리 전 400 | 구현 예정 |
| `C2-SRCH-FILTER-001` | `검색-필터-01` | HTTP·조합 | `status=available\|sold`, `min_price`, `max_price` 허용 경계와 잘못된 상태·가격 | 허용 필드는 AND 결합한 HTML 200, 잘못된 값은 상품 쿼리 전 HTML 400 | 구현 예정 |
| `C2-SRCH-ORDER-001` | `검색-정렬-01` | HTTP·결정성 | 생략/`newest`, `price_asc`, `price_desc`, 동률, 임의 값, 반복 조회 | 세 wire 값만 HTML 200이고 ID 내림차순 tie-break, 그 밖은 400 | 구현 예정 |
| `C2-SRCH-PAGE-001` | `검색-페이지-01` | HTTP·페이지 | `page` 생략·1·2·3·500·501·매우 큰 값·소수·문자 | 1..500은 HTML 200, 501/형식 오류는 상품 쿼리 전 HTML 400, 결과 ID·건수 정확 | 구현 예정 |
| `C2-SRCH-RESOURCE-001` | `검색-입력-01`, `검색-페이지-01` | 자원 제한 | `CaptureQueriesContext`를 검색 서비스 호출에만 둘러 0·20·대량 결과의 500쪽과 501쪽·매우 큰 값을 측정 | middleware/auth/session 제외 서비스 블록에서 1..500은 count+slice 최대 2회·20행, 501 이상은 상품 쿼리 0회 | 구현 예정 |
