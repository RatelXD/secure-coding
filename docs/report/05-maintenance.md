...
이 장에서는 배포 전 개발·검증 과정에서 수정한 결함과 정식 릴리스 뒤 유지관리자가 관찰해 수정한 결함을 구분해 기록합니다. 5.2는 배포 전 점검 사례이며, 5.3은 실제 사용자 신고가 아니라 정식 릴리스에 대한 유지관리자 검증에서 발견한 사례입니다.
...
이 표의 ‘발견 당시 상태’는 수정 전 기록입니다. 현재 상태를 뜻하지 않으며, 각 항목은 수정 후 표의 ‘확인 결과’에 적힌 방식으로 다시 점검했습니다.
...
### MNT-05. 상품 채팅 연결 상태와 송금 표시 보정

| 항목 | 내용 |
|---|---|
| 사례 ID | `MNT-05` |
| 발견 환경 | main `03f0f1e` 이후 Stitch 채팅 화면과 상품 대화 검토 |
| 문제 현상 | WebSocket이 연결되어도 화면의 presence가 `상품 대화 · 연결 중`에 머물렀고, 비정상 종료가 반복되면 재연결 카운터가 초기화될 수 있었습니다. 송금 화면과 알림에는 소수 표현이 남아 있었습니다. |
| 원인 | 초기 화면 문구와 실제 `open` 이벤트 상태 갱신이 분리되어 있었고, 연결 직후마다 재시도 횟수를 0으로 되돌렸습니다. UI·API·알림이 서로 다른 금액 표현을 사용했습니다. |
| 수정 내용 | PR #43에서 연결·종료 상태, 최대 5회 지수 백오프, 가로형 공통 로고, 정수 원화 입력·응답·알림을 반영했습니다. PR #44에서 10초 안정 연결 뒤에만 재연결 카운터를 초기화하도록 보정했습니다. |
| 검증 | PR #43 run `30038923030`, PR #44 run `30039390145`에서 필수 CI 6개가 각각 PASS했습니다. |
| 잔여 위험 | 실제 배포 프록시의 WebSocket upgrade·close code 보존과 로컬 서버를 사용한 수동 끊김·복구 캡처는 별도 확인이 필요합니다. |
...
4. 원인을 직접 수정하되 관련 없는 범위는 바꾸지 않습니다.
5. 같은 문제가 재발하지 않도록 음성 테스트 또는 회귀 테스트를 추가합니다.
...
이후 실제 문제가 발견되면 아래 표를 복사해 작성합니다.
...
보안 관련 문제는 [보안 약점과 개선 계획](06-security-improvements.md)의 항목과 연결합니다. 테스트 명령과 캡처는 [테스트 근거](appendix/test-evidence.md)에 남기며, 최종 PDF에는 비밀값·개인정보·내부 경로가 보이지 않도록 정리합니다.
