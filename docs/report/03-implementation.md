...
- `src/apps/trades/services.py`, `models.py`
  - 세션 행위자와 DB 사용자 상태를 다시 확인하고 User→Product→Trade 잠금 순서에서 예약·취소·완료를 전이합니다.
  - 상품 호환 필드를 쓰지 않고 버전별 상태 이력을 보존하며, PostgreSQL trigger가 이력 변경·삭제를 거부합니다.
- `src/apps/transfers/models.py`, `services.py`, `views.py`
  - 회원별 `100,000.00` 모의 계정을 합계 0인 `SEED_ISSUE` 분개와 함께 정확히 한 번 생성합니다.
  - safety shared advisory lock, 발신자 범위 멱등 lock, 계정 PK 오름차순 잠금으로 이체하며 세션 사용자를 발신자로 고정합니다.
  - 성공과 업무 거부 응답을 영구 재현하고 payload 변경은 거부합니다. journal은 정확히 두 항목·합계 0을 커밋 시 검사하며 journal/entry 변경·삭제를 DB trigger로 막습니다.
  - exclusive 대사 command는 불일치 시 incident와 함께 신규 이체를 차단하고 정확한 incident, 불일치 0건, 전용 DB 역할에서만 재개합니다.
