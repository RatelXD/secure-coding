...
이 장은 실제 코드 경로가 확인되는 내용과 앞으로 구현할 내용을 구분합니다. 2026-07-16 기준 사용자·상품·채팅·신고·가역 제재의 URL, 화면, 서비스와 보안 회귀 테스트를 구현했습니다. 유지보수 수정 후 전체 자동 테스트 168개와 하위 사례 216개가 통과했으며, 컨테이너 재시작과 데이터베이스 백업·복구도 확인했습니다.
