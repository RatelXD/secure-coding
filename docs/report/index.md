...
이 문서는 시큐어 코딩 과제로 진행하는 중고거래 플랫폼의 요구사항 분석, 시스템 설계, 구현, 테스트, 유지보수 과정을 한곳에 정리한 보고서입니다. 최종 제출본은 이 Markdown 문서들을 바탕으로 사용자가 직접 PDF로 편집합니다.
...
| 구현 | 통합 구현 완료·최종 검증 보정 | G005~G016 제품 기능과 G017 검증기를 구현했습니다. |
...
| 최종 제출 자료 | G018 보고서 갱신 | 한국어 보고서와 추적표를 실제 실행 근거로 갱신했으며 tag·release·LMS는 G019에서만 수행합니다. |
