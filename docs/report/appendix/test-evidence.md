...
위 결과는 실제로 실행했습니다. 이미지 재인코딩과 신고 동시성 불변식은 통과했지만 채팅 수락 경로의 PostgreSQL 행 잠금 결함과 형식이 잘못된 Host authority를 허용하는 정확 Origin 결함을 발견했습니다. 두 결함을 수정한 뒤 같은 독립 테스트와 모든 변경을 합친 전체 스위트가 통과하기 전에는 1차 전체 기능을 PASS로 해석하지 않습니다. 검색·관리자·모의 이체는 2차 범위이며 구현하지 않았습니다.

## B.2 결과 해석

기존 자동 테스트는 아이디 정규화, 비밀번호 단방향 해시 저장, 상품 소유자 관계, 채팅 입력·재전송 정책, 신고 임계값 정책, 프록시 헤더 신뢰 경계를 확인합니다. 새 독립 테스트는 인증·IDOR·CSRF·XSS, 비밀번호 저장·로그, 이미지 우회, WebSocket Origin·참여자, 재전송·속도·Redis 장애 수렴, 동시 제재·만료, 일반 오류, 마이그레이션 leaf와 2차 기능 부재를 다룹니다. 전체 통합 실행 전에는 새 항목을 `재검증 필요`로 유지합니다.

## B.3 남은 자동화 테스트

| 영역 | 필요한 테스트 | 상태 |
|---|---|---|
| 사용자 | 회원가입·로그인, 일반 오류, 비밀번호 저장·로그, CSRF, 타인 변경 거부 | 테스트 추가, 통합 실행 필요 |
| 상품 | 가격 경계, 소유자 CRUD, 공개 여부, 재인코딩·메타데이터·우회 입력 | 테스트 추가, 통합 실행 필요 |
| 채팅 | 인증·Origin·참여자, 재전송 충돌, 속도, Redis 장애·이력 동기화 | 테스트 추가, 통합 실행 필요 |
| 신고·제재 | 필수 사유, 자기 신고, 동시 임계값, 단일 감사, 정확한 만료 | 테스트 추가, 통합 실행 필요 |
