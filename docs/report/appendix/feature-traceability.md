...
이 표는 요구사항이 설계와 코드, 테스트에 어떻게 연결되는지 보여줍니다. 공개 `main`에 없는 개발 브랜치 코드는 `구현 중`으로 표시하고 통합 뒤 다시 확인합니다.

| 요구사항 ID | 요구사항 | 설계 요소 | 구현 파일·모듈 | 테스트 근거 | 상태 | 비고 |
|---|---|---|---|---|---|---|
| `FR-USER-01` | 회원가입·로그인 | Django 인증·세션, 아이디 정규화 | `apps/accounts` | 사용자 골격 단위 테스트 재검증 필요 | 구현 중 | 로그인 URL·속도 제한 미구현 |
...
| `FR-PRODUCT-01` | 상품 등록 | 소유자 관계·안전 이미지 파이프라인 | `apps/catalog` | 상품 골격 단위 테스트 재검증 필요 | 구현 중 | 가격·재인코딩 확인 필요 |
...
| `SR-AUTH-01` | 비밀번호 안전 저장 | Django hash·validator | `apps/accounts` 예정 | DB·로그 평문 검색 예정 | 구현 예정 | 가입 서비스 필요 |
...
| `SR-SESSION-01` | 세션·CSRF·Host·HTTPS | Django 보안 설정 | `config/settings.py` 개발 중 | `check --deploy`, CSRF 테스트 예정 | 구현 중 | 공개 `main` 통합 후 확인 |
...
2. 공개 `main`에 통합된 코드와 실제 테스트 결과를 기준으로 갱신합니다.
